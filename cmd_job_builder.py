...
class Config:
    ...
    po_parallelism: int = 2  # Max concurrent PO processing (reduces disk/PDF contention)
    zip_compression: str = "deflated"  # "stored" (fast) or "deflated" (smaller)
...
def load_config(path: Path) -> Config:
    ...
    po_parallelism = int(cfg.get("po_parallelism", 2))
    zip_compression = str(cfg.get("zip_compression", "deflated")).lower()
...
def preflight_pdf_backend() -> None:
    """Check that pdfplumber is available before attempting PDF operations."""
    try:
        import pdfplumber
    except ImportError:
        raise SystemExit("Missing dependency pdfplumber. Run: pip install -r requirements.txt")


def read_pdf_text(pdf_path: Path, max_pages: Optional[int] = None) -> str:
    """Read text from PDF, optionally limiting to first max_pages pages."""
    try:
        with pdfplumber.open(str(pdf_path)) as pdf:
            ...
            if len(text.strip()) < 50:
                print(f"Warning: little/no extractable text in {pdf_path.name}. PDF may be scanned.")
            return text
    except Exception as e:
        raise SystemExit(f"Failed to read PDF text from {pdf_path}: {e}")


def read_pdf_text_all_pages(pdf_path: Path) -> str:
    """Read all pages from PDF (backwards compatibility)."""
    return read_pdf_text(pdf_path, max_pages=None)


def extract_po_number_from_text(text: str, pdf_path: Path, logger: logging.Logger) -> str:
...
def extract_po_number_from_pdf(pdf_path: Path, logger: logging.Logger) -> str:
    """Extract PO number from PDF (backwards compatibility)."""
    text = read_pdf_text(pdf_path, max_pages=3)  # Try first 3 pages first
    try:
        return extract_po_number_from_text(text, pdf_path, logger)
    except SystemExit:
        # Fallback to all pages if not found in first 3
        text = read_pdf_text(pdf_path, max_pages=None)
        return extract_po_number_from_text(text, pdf_path, logger)
...
def parse_items_from_po_text(text: str, pdf_path: Path, logger: logging.Logger) -> List[ItemRow]:
...
def parse_items_from_po_pdf(pdf_path: Path, logger: logging.Logger) -> List[ItemRow]:
    """Parse items from PDF (backwards compatibility)."""
    text = read_pdf_text(pdf_path, max_pages=3)  # Try first 3 pages first
    items = parse_items_from_po_text(text, pdf_path, logger)
    if not items:
        # Fallback to all pages if no items found in first 3
        text = read_pdf_text(pdf_path, max_pages=None)
        items = parse_items_from_po_text(text, pdf_path, logger)
    return items


def extract_versions_sections_from_text(text: str, pdf_path: Path, logger: logging.Logger) -> Dict[str, str]:
    """Extract version sections from PDF text."""
...
def extract_versions_sections(pdf_path: Path, logger: logging.Logger) -> Dict[str, str]:
    """Extract version sections from PDF (backwards compatibility)."""
    text = read_pdf_text(pdf_path, max_pages=None)  # Versions usually at end
    return extract_versions_sections_from_text(text, pdf_path, logger)
...
def fast_copytree(src: Path, dst: Path) -> None:
    """Fast copy using hardlinks when possible, falls back to regular copy."""
    dst.mkdir(parents=True, exist_ok=True)
    for item in src.iterdir():
        src_item = src / item.name
        dst_item = dst / item.name
        if src_item.is_dir():
            fast_copytree(src_item, dst_item)
        else:
            try:
                # Try hardlink first (much faster, same filesystem only)
                os.link(src_item, dst_item)
            except (OSError, AttributeError):
                # Fallback to regular copy (cross-filesystem or Windows)
...
def copy_sc_folder(source_dir: Path, dest_dir: Path, reset: bool, logger: logging.Logger, dry_run: bool) -> None:
    ...
    if not dest_dir.exists():
        if not dry_run:
            # Use fast_copytree for hardlink optimization
            fast_copytree(source_dir, dest_dir)
    else:
...
def generate_coc(
    ...
    for idx, it in enumerate(items, start=1):
        ...
        # Leave Traveler Number blank per row
        add_replacement(f"{{{{Traveler Number {idx}}}}}", "")
...
def zip_job_folder(job_dir: Path, logger: logging.Logger, compression: str = "deflated") -> Path:
    parent = job_dir.parent
    zip_path = parent / f"{job_dir.name}.zip"
    if zip_path.exists():
        zip_path.unlink()
    
    zip_compression_mode = zipfile.ZIP_STORED if compression == "stored" else zipfile.ZIP_DEFLATED
    zip_start_time = time.time()
    with zipfile.ZipFile(zip_path, "w", zip_compression_mode) as z:
    ...
    logger.info(f"Created zip: {zip_path} ({zip_duration:.2f}s, compression={compression})")
    return zip_path
...
def process_po_source(
    ...
    # Read PO PDF text ONCE and reuse for all operations
    # Try first 3 pages for PO number and items (most common case)
    po_text_3pages = read_pdf_text(po_pdf, max_pages=3)
    try:
        po_number = extract_po_number_from_text(po_text_3pages, po_pdf, logger)
        po_text = po_text_3pages
    except SystemExit:
        # Fallback to all pages if not found in first 3
        po_text = read_pdf_text(po_pdf, max_pages=None)
        po_number = extract_po_number_from_text(po_text, po_pdf, logger)
    ...
    # Parse PO items from cached text
    items = parse_items_from_po_text(po_text, po_pdf, logger)
    if not items:
        # Fallback to all pages if no items found in first 3
        po_text = read_pdf_text(po_pdf, max_pages=None)
        items = parse_items_from_po_text(po_text, po_pdf, logger)
    
    # Extract versions from cached text (usually need full PDF)
    # If we only have 3 pages worth of text, read all for versions
    if len(po_text.splitlines()) < 100:
        po_text = read_pdf_text(po_pdf, max_pages=None)
    version_map = extract_versions_sections_from_text(po_text, po_pdf, logger)
    resolve_item_versions(items, version_map, logger)
    
    po_duration = time.time() - po_start_time
    logger.debug(f"PO {po_number} processed in {po_duration:.2f}s")
...
def run_builder(cfg: Config, verbose: bool, dry_run: bool, progress_callback: Optional[callable] = None) -> None:
    ...
    # Reduced to 2 workers by default to reduce disk/PDF contention (configurable via po_parallelism)
    max_workers = min(cfg.po_parallelism, len(sources))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        future_to_src = {executor.submit(process_one_po, src): src for src in sources}
        ...
        for future in as_completed(future_to_src):
            ...
            try:
                ...
                if error:
                    any_errors.append(f"{name}: {error}")
                    run_report_lines.append(f"ERR: {name}: {error}")
                else:
                    run_report_lines.append(f"OK: {name}")
            except Exception as e:
    ...
    if not dry_run:
        ...
        if cfg.output_root and cfg.output_root.exists():
            ...
            zip_path = zip_job_folder(job_dir, logger, compression=cfg.zip_compression)
            ...
            zip_job_folder(job_dir, logger, compression=cfg.zip_compression)
...
# python -c "import pdfplumber; from cmd_job_builder import read_pdf_text_all_pages; print('ok')"
